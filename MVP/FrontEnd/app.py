...
# -----------------------------------------------------------------------------
# 1. PAGE CONFIGURATION & LIGHTER DARK THEME CSS
# -----------------------------------------------------------------------------
# ✅ ONLY ONE st.set_page_config() call
st.set_page_config(
...
<style>
    ...
    /* ── CSS Variables — lighter dark palette ───────────────────────── */
    :root {
        --bg:        #0f1923;
        --surface:   #16202c;
        --surface2:  #1c2b38;
        ...
        --text-sub:  #8fa3b8;
        --text-dim:  #4a6074;
        --glow:      0 0 40px rgba(0,210,180,0.12);
    ...
    section[data-testid="stSidebar"] {
        background: linear-gradient(180deg, #131e28 0%, #0f1923 100%) !important;
    ...
    /* ── Cards ──────────────────────────────────────────────────────── */
    .css-card {
        background: linear-gradient(135deg, #1c2b38 0%, #16202c 100%);
    ...
    .css-card::before {
        ...
        height: 1px;
        background: linear-gradient(90deg, transparent, rgba(0,210,180,0.45), transparent);
        opacity: 0;
    ...
    /* ── Buttons ────────────────────────────────────────────────────── */
    div.stButton > button {
        background: linear-gradient(135deg, #00d2b4 0%, #3b82f6 100%) !important;
        ...
        box-shadow: 0 4px 16px rgba(0,210,180,0.2) !important;
    }
    div.stButton > button:hover {
        transform: translateY(-2px) !important;
        box-shadow: 0 8px 28px rgba(0,210,180,0.35) !important;
        color: #07090f !important;
    }
    ...
    .stNumberInput > div > div > input:focus {
        border-color: var(--border-hi) !important;
        box-shadow: 0 0 0 3px rgba(0,210,180,0.10) !important;
    }
    ...
    ::-webkit-scrollbar-thumb { background: #2a3d50; border-radius: 999px; }
    ...
    .stSuccess { background: rgba(0,210,180,0.10) !important; border-color: rgba(0,210,180,0.3) !important; }
    .stWarning { background: rgba(245,158,11,0.10) !important; border-color: rgba(245,158,11,0.3) !important; }
    .stError   { background: rgba(239,68,68,0.10)  !important; border-color: rgba(239,68,68,0.3)  !important; }
    .stInfo    { background: rgba(59,130,246,0.10)  !important; border-color: rgba(59,130,246,0.3)  !important; }
    ...
    [data-baseweb="popover"] { background-color: #1c2b38 !important; }
    [data-baseweb="menu"]    { background-color: #1c2b38 !important; }
    [data-baseweb="option"]:hover { background-color: rgba(0,210,180,0.1) !important; }
    ...
    .stProgress > div > div > div > div {
        background: linear-gradient(90deg, #00d2b4, #3b82f6) !important;
    }
...
# ✅ Single initialisation — no duplicate
...
if "tasks_df" not in st.session_state:
    data = {
        "Task": ["Final Year Project Proposal", "Machine Learning Assignment 1",
                 "Research Methodology Review", "Course Registration"],
...
def is_valid_against_csv(new_row):
    ...
    required_cols = [
        ...
        "Avg_Delay_History","Urgency","Task_Type_Exam","Task_Type_Project",
        ...
        if model:
            def predict_priority(row: pd.Series) -> str:
                ...
                features   = {
                    ...
                    "Current_Workload": 6.0, "Procrastination_Score": 0.5, "Avg_Delay_History": 0.3,
                    "Urgency": urgency,
                ...
                cols  = ["Weeks_Left","Week_Released","Week_Deadline","Current_Week","Semester","Year",
                         "Weight","Difficulty","Estimated_Hours","Current_Workload",
                         "Procrastination_Score","Avg_Delay_History","Urgency",
                         "Task_Type_Exam","Task_Type_Project","Task_Type_Quiz","Task_Type_Report"]
...
if not st.session_state.login_state:
    ...
    with col2:
        ...
        st.markdown("""
        <div style="
            background: linear-gradient(160deg, #16202c 0%, #1c2b38 100%);
            padding: 48px 40px 36px;
            border-radius: 20px;
            border: 1px solid rgba(0,210,180,0.18);
            text-align: center;
            box-shadow: 0 24px 64px rgba(0,0,0,0.45), 0 0 60px rgba(0,210,180,0.06);
            position: relative;
            overflow: hidden;
        ">
            <div style="
            ...
            <div style="
                ...
                background: linear-gradient(135deg, rgba(0,210,180,0.18), rgba(59,130,246,0.18));
            ...
            <p style="color: #8fa3b8; font-size: 0.82rem; line-height: 1.6; margin: 0 0 24px;">
                Informatics Institute of Technology<br>
                <span style="color: #5c7a92;">Robert Gordon University Aberdeen</span>
            </p>
...
with st.sidebar:
    ...
    <div style="padding: 0 4px 24px;">
        <div style="
            ...
            background: linear-gradient(135deg, rgba(0,210,180,0.10), rgba(59,130,246,0.08));
            border: 1px solid rgba(0,210,180,0.15);
            margin-bottom: 8px;
        ">
            <div style="
                width: 36px; height: 36px; border-radius: 10px; font-size: 18px;
                background: rgba(0,210,180,0.14); border: 1px solid rgba(0,210,180,0.25);
                display: flex; align-items: center; justify-content: center; flex-shrink: 0;
            ">🎓</div>
            <div>
                <div style="font-family:'Figtree',sans-serif; font-weight:700; font-size:0.95rem; color:#e8edf2; line-height:1.2;">Domain Specific AI</div>
    ...
    ">
        ...
        <div>
            <div style="font-size:0.65rem; letter-spacing:0.12em; text-transform:uppercase; color:#4a6074; margin-bottom:1px;">Current Role</div>
            <div style="font-family:'Figtree',sans-serif; font-weight:700; font-size:0.9rem; color:#e8edf2;">{st.session_state.user_role}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
">
    <div>
        ...
        <h2 style="font-family:'Figtree',sans-serif; font-weight:800; font-size:1.9rem; margin:0; letter-spacing:-0.03em; color:#e8edf2;">{menu}</h2>
        <p style="color:#5c7a92; margin:4px 0 0; font-size:0.82rem;">
            Logged in as <span style="color:#8fa3b8; font-weight:600;">{st.session_state.user_role}</span>
            &nbsp;·&nbsp; Academic Year 2025/26
        </p>
    </div>
    <div style="text-align:right;">
        <div style="
            font-family:'JetBrains Mono',monospace; font-size:0.75rem; color:#4a6074;
...
if menu == "Dashboard":
    ...
    risk_grad  = ('linear-gradient(90deg,#ff3b30,#ff6b6b)' if risk_val > 80
                  else 'linear-gradient(90deg,#f5a623,#fbbf24)' if risk_val > 40
                  else 'linear-gradient(90deg,#00d2b4,#3b82f6)')
    ...
    with m1:
        ...
        <div class="css-card" style="padding:20px 22px;">
            ...
            <div style="font-size:0.7rem; color:#4a6074; margin-top:6px; font-weight:500;">AI Prediction Model</div>
        </div>""", unsafe_allow_html=True)

    with m2:
        deadline_color = '#ff3b30' if days_left <= 2 else '#f5a623' if days_left <= 5 else '#e8edf2'
        st.markdown(f"""
        <div class="css-card" style="padding:20px 22px;">
            ...
            <div style="font-size:0.75rem; color:#8fa3b8; margin-top:4px;">{f'{days_left}d remaining' if days_left >= 0 else 'Overdue'}</div>
        </div>""", unsafe_allow_html=True)
    ...
    with m3:
        ...
        <div class="css-card" style="padding:20px 22px;">
            ...
            <div class="metric-value" style="color:#e8edf2; font-size:1.55rem; margin:6px 0 4px;">{len(pending_df)}</div>
            <div style="font-size:0.75rem; color:#8fa3b8; margin-top:4px;">{len(done_df)} completed · {len(df)} total</div>
        </div>""", unsafe_allow_html=True)
    ...
    with m4:
        ...
        <div class="css-card" style="padding:20px 22px;">
            ...
            <div style="height:3px; border-radius:999px; background:rgba(255,255,255,0.06); overflow:hidden;">
                <div style="height:100%; width:{completion_pct}%; border-radius:999px; background:linear-gradient(90deg,#00d2b4,#3b82f6);"></div>
            </div>
            <div style="font-size:0.7rem; color:#4a6074; margin-top:6px; font-weight:500;">Sprint Completion</div>
        </div>""", unsafe_allow_html=True)
    ...
    with c1:
        ...
        st.markdown("<div style='font-size:0.7rem;letter-spacing:0.1em;text-transform:uppercase;color:#8fa3b8;font-weight:600;margin-bottom:2px;'>Stress Gauge</div>", unsafe_allow_html=True)
        fig_gauge = go.Figure(go.Indicator(
            mode="gauge+number", value=risk_val,
            domain={'x':[0,1],'y':[0,1]},
            gauge={
                'axis':{'range':[None,100],'tickcolor':"#4a6074",'tickwidth':1,'tickfont':{'color':'#4a6074','size':9}},
                'bar':{'color':risk_color,'thickness':0.25},
                'bgcolor':PLOT_BG,'borderwidth':0,
                'steps':[
                    {'range':[0,50],  'color':"rgba(0,210,180,0.07)"},
                    {'range':[50,80], 'color':"rgba(245,166,35,0.07)"},
                    {'range':[80,100],'color':"rgba(255,59,48,0.09)"}
                ],
            },
            number={'font':{'color':risk_color,'family':FONT_FAM,'size':28},'suffix':''}
        ))
        fig_gauge.update_layout(paper_bgcolor=PLOT_BG, font={'color':FONT_COL,'family':FONT_FAM},
    ...
    with c2:
        st.markdown('<div class="css-card" style="padding:20px;">', unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.7rem;letter-spacing:0.1em;text-transform:uppercase;color:#8fa3b8;font-weight:600;margin-bottom:2px;'>Priority Split</div>", unsafe_allow_html=True)
        fig_pie = go.Figure(go.Pie(
            labels=['Critical','High','Medium','Low'],
            values=[max(critical_n,0.01),max(high_n,0.01),max(medium_n,0.01),max(low_n,0.01)],
            hole=0.68,
            marker=dict(colors=['#ff3b30','#f5a623','#3b82f6','#00d2b4'],
                        line=dict(color='#16202c',width=2)),
            textinfo='none',
        ...
        fig_pie.add_annotation(text=f"{len(df)}<br><span style='font-size:9px'>tasks</span>",
                               ...
                               font=dict(size=18,color='#e8edf2',family=FONT_FAM))
        fig_pie.update_layout(paper_bgcolor=PLOT_BG, showlegend=False,
                              height=200, margin=dict(l=16,r=16,t=16,b=0))
    ...
    with c3:
        st.markdown('<div class="css-card" style="padding:20px;">', unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.7rem;letter-spacing:0.1em;text-transform:uppercase;color:#8fa3b8;font-weight:600;margin-bottom:12px;'>Task Urgency by Deadline</div>", unsafe_allow_html=True)
        plot_df = df[df['Status'] != 'Done'].sort_values('_urgency', ascending=False).head(6)
        if not plot_df.empty:
            pcolor_map = {'Critical':'#ff3b30','High':'#f5a623','Medium':'#3b82f6','Low':'#00d2b4'}
            bar_colors = [pcolor_map.get(p,'#8fa3b8') for p in plot_df['Priority']]
            fig_bar = go.Figure(go.Bar(
                x=plot_df['_urgency'], y=plot_df['Task'].str[:28],
                orientation='h',
                marker=dict(color=bar_colors, line=dict(width=0)),
                text=[f"{v:.2f}" for v in plot_df['_urgency']],
                textposition='outside',
                textfont=dict(color=FONT_COL,size=10,family=FONT_FAM),
                hovertemplate='<b>%{y}</b><br>Urgency: %{x:.2f}<extra></extra>'
            ))
            fig_bar.update_layout(
                paper_bgcolor=PLOT_BG, plot_bgcolor=PLOT_BG,
                font={'color':FONT_COL,'family':FONT_FAM},
                xaxis=dict(range=[0,1.15],gridcolor=GRID_COL,zerolinecolor="rgba(0,0,0,0)",tickfont=dict(size=10)),
                yaxis=dict(gridcolor="rgba(0,0,0,0)",tickfont=dict(size=10,color=FONT_COL)),
                height=200, margin=dict(l=8,r=40,t=4,b=4), bargap=0.35
            )
            st.plotly_chart(fig_bar, use_container_width=True)
        else:
            st.markdown("<p style='color:#8fa3b8;font-size:0.85rem;padding:20px 0;'>All tasks completed 🎉</p>", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    # ── Row 3: Priority breakdown + Velocity ─────────────────────────
    st.markdown("<div style='margin-top:8px;'></div>", unsafe_allow_html=True)
    r1, r2 = st.columns([1, 2])

    with r1:
        st.markdown('<div class="css-card" style="padding:20px;">', unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.7rem;letter-spacing:0.1em;text-transform:uppercase;color:#8fa3b8;font-weight:600;margin-bottom:14px;'>ML Priority Breakdown</div>", unsafe_allow_html=True)
        for label, count, color in [("Critical",critical_n,"#ff3b30"),("High",high_n,"#f5a623"),("Medium",medium_n,"#3b82f6"),("Low",low_n,"#00d2b4")]:
            ...
            <div style="margin-bottom:12px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                    ...
                    <span style="font-family:'JetBrains Mono',monospace;font-size:0.75rem;color:#8fa3b8;">{count} · {pct}%</span>
                </div>
                <div style="height:3px;border-radius:999px;background:rgba(255,255,255,0.06);overflow:hidden;">
    ...
    with r2:
        st.markdown('<div class="css-card" style="padding:20px;">', unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.7rem;letter-spacing:0.1em;text-transform:uppercase;color:#8fa3b8;font-weight:600;margin-bottom:12px;'>Task Velocity This Week</div>", unsafe_allow_html=True)
        chart_data = pd.DataFrame({
            "Day":["Mon","Tue","Wed","Thu","Fri"],
            "Completed":[2,1,3,0,4],"Added":[1,2,1,5,2]
        })
        fig_vel = px.bar(chart_data, x="Day", y=["Completed","Added"],
                         barmode='group', color_discrete_sequence=["#00d2b4","#3b82f6"])
        fig_vel.update_layout(
            paper_bgcolor=PLOT_BG, plot_bgcolor=PLOT_BG,
            font={'color':FONT_COL,'family':FONT_FAM},
            xaxis=dict(gridcolor=GRID_COL,zerolinecolor="rgba(0,0,0,0)",tickfont=dict(size=10)),
            yaxis=dict(gridcolor=GRID_COL,zerolinecolor="rgba(0,0,0,0)",tickfont=dict(size=10)),
            legend=dict(orientation="h",yanchor="bottom",y=1.02,xanchor="right",x=1,
                        font=dict(color=FONT_COL,size=10),bgcolor="rgba(0,0,0,0)"),
...
# -----------------------------------------------------------------------------
    ...
    st.markdown(
        f"<div style='font-family:\"JetBrains Mono\",monospace;font-size:0.78rem;color:#4a6074;"
        f"background:rgba(255,255,255,0.03);padding:8px 14px;border-radius:8px;"
        f"border:1px solid rgba(255,255,255,0.06);display:inline-block;margin-bottom:14px;'>🕒  {now}</div>",
        unsafe_allow_html=True
    )
            ...
            if st.form_submit_button("Add to Workflow"):
                ...
                if errors:
                    ...
                    st.session_state.tasks_df = pd.concat(
                        [st.session_state.tasks_df, new_data], ignore_index=True
                    )
...
# -----------------------------------------------------------------------------
    ...
    def get_user_inputs(target_year, current_features):
        ...
        hours_mapping = {'Less than 3hrs':1.5,'3 - 5 hrs':4.0,'5 - 10 hrs':7.5,'More than 10hrs':12.5}
    ...
    prediction_task = st.selectbox(
        "Select Target Prediction Year:", ["Predict 3rd Year GPA","Predict 4th Year GPA"]
    )
    ...
    st.markdown("---")
    st.markdown(
        "<div style='text-align:center;color:#5c7a92;'>"
...
# -----------------------------------------------------------------------------
    ...
    # ✅ NO duplicate st.set_page_config() — removed
    # ✅ NO duplicate <style> block — global CSS covers all styling
            ...
            with st.spinner('Analyzing peer patterns...'):
                try:
                    ...
                    <div class="result-box">
                        <h3 style='color:#00d2b4; margin:0 0 8px;'>Top Pick: {rec_course}</h3>
                        <p style='color:#8fa3b8; margin:0;'>Students with a similar GPA profile to yours performed best in this elective.</p>
                    </div>
                    """, unsafe_allow_html=True)
