...
# -----------------------------------------------------------------------------
    ...
    with col_info:
        ...
        prompts = [
            ...
            "Can you explain the syllabus?",
            "Summarize the module content."
        ]
        for p in prompts:
        ...
        st.info("System connected to University Knowledge Base (Backend API)")
        ...
        if user_input := st.chat_input("Ask the Domain AI..."):
            ...
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    # Construct full response with sources
                    full_resp = answer
                    if sources:
                        full_resp += "\n\n**Sources:**\n"
                        for idx, source in enumerate(sources, 1):
                            ...
                            full_resp += f"- [{idx}] {doc} (Page {page})\n"

                except requests.exceptions.ConnectionError:
                    full_resp = "❌ Error: Could not connect to the backend server. Is it running on port 8000?"
                except Exception as e:
                    full_resp = f"❌ Error: An unexpected error occurred: {str(e)}"
                
                # Stream Response
                streamed_resp = ""
                for chunk in stream_text(full_resp):
                    streamed_resp += chunk
                    placeholder.markdown(streamed_resp + "▌")
                placeholder.markdown(streamed_resp)
            ...
            for r in recs:
                ...
                <div class="css-card" style="border-left: 4px solid #00ADB5;">
                    ...
                    <div style="margin-top:10px;">
                        {''.join([f'<span style="background:#4A5568; color:white; padding:2px 8px; border-radius:4px; font-size:12px; margin-right:5px;">{t}</span>' for t in list(r.get('tags', []))])}
                    </div>
                </div>
                """, unsafe_allow_html=True)
